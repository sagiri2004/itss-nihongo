...
from ...pdf_processing.gemini_processor import GeminiProcessor
...
class SlideProcessingRequest(BaseModel):
    ...
    use_llm_summary: bool = Field(True, alias="use_llm_summary")  # Force use LLM for summary
...
class SlideDetails(BaseModel):
    """Slide-level information returned to the backend (simplified - Gemini only)."""

    slide_id: int = Field(..., alias="slide_id")
    keywords: List[str] = Field(default_factory=list)  # Keywords extracted by Gemini
    summary: str = Field("", alias="summary")  # Summary generated by Gemini
...
class SlideProcessingResponse(BaseModel):
    """Response returned after processing slides stored in GCS (simplified - Gemini only)."""

    lecture_id: int = Field(..., alias="lecture_id")
    original_name: Optional[str] = Field(None, alias="original_name")
    slide_count: int = Field(..., alias="slide_count")
    keywords_count: int = Field(..., alias="keywords_count")
    all_summary: str = Field("", alias="all_summary")  # Global summary generated by Gemini
    slides: List[SlideDetails]  # Simplified: only slide_id, keywords, summary
...
def _generate_all_summary(processor: SlideProcessor, use_llm: bool = True) -> str:
    """
    Generate global summary for all slides using TextSummarizer.
    
    Args:
        processor: SlideProcessor instance with processed slides
        use_llm: Whether to force use LLM for summarization (default: True)
    ...
    """
    try:
        # Force use LLM if requested and available
        summarizer = TextSummarizer(use_llm=use_llm)
...
async def upload_slide(
    ...
    use_embeddings: bool = True,  # Not used with Gemini, kept for compatibility
    use_llm_summary: bool = True,  # Not used with Gemini, kept for compatibility
) -> Dict[str, Any]:
    """Receive a PDF, process it with Gemini API and return JSON data."""
    ...
    try:
        ...
        # Use Gemini processor instead of local processing
        ...
        response: Dict[str, Any] = {
            ...
            "all_summary": result.get("all_summary", ""),
...
async def process_slide(request: SlideProcessingRequest) -> SlideProcessingResponse:
    """Process a slide deck stored in GCS using Gemini API and return structured data."""
    temp_path: Optional[Path] = None
    try:
        ...
        # Build slide models from Gemini result
        slide_models = [
            SlideDetails(
                slide_id=slide["slide_id"],
                keywords=slide.get("keywords", []),
